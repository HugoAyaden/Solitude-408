...
/**
 * \brief Sélectionne la texture de fond selon la caméra active et la position du monstre/mimic.
 *
 * Correspondance bouton → caméra (numérotation map.c) :
 *   cameras_on[0] → bouton 1 → CAMERA_5  (couloir droite)
 *   cameras_on[1] → bouton 2 → CAMERA_15 (couloir gauche)
 *   cameras_on[3] → bouton 4 → CAMERA_8  (corridor central)
 *   cameras_on[7] → bouton 8 → CAMERA_1  (salle moniteur)
 *   cameras_on[8] → bouton 9 → CAMERA_12 (salle commande)
 * Les autres caméras n'ont pas encore de texture dédiée → affiche STATIC.
 */
void change_camera(case_t * camera, case_t * monster){
    ...
    if(cameras_on[7] && camera->num_camera == CAMERA_1){
    ...
    else if(cameras_on[0] && camera->num_camera == CAMERA_5){
    ...
    else if(cameras_on[1] && camera->num_camera == CAMERA_15){
    ...
    else if(cameras_on[3] && camera->num_camera == CAMERA_8){
    ...
    else if(cameras_on[8] && camera->num_camera == CAMERA_12){
...
{
    return panneau.porte_gauche + panneau.porte_droite;
}
...
{
    return panneau.lumiere_gauche + panneau.lumiere_droite;
}
...
{
    /* La durée totale diminue avec chaque nuit (nuit 0 : 2000s, nuit 1 : 1500s, nuit 2 : 1000s) */
    BATTERY_DURATION = 2000.0f - night * 500;
    float drainRate = 100.0f / BATTERY_DURATION;

    /* Multiplicateurs de consommation selon l'utilisation des portes + moniteur */
    if (doorCount == 2 && cameraButton())
        drainRate *= 4.0f;   /* 2 portes + moniteur caméra : consommation maximale */
    else if (doorCount == 2 && !cameraButton())
        drainRate *= 3.0f;   /* 2 portes seules */
    else if (doorCount == 1)
        drainRate *= 2.0f;   /* 1 porte */

    /* Chaque lumière allumée ajoute encore un facteur (1 lumière = ×2, 2 lumières = ×3) */
    if (lightCount > 0)
...
/*=====NE JAMAIS ENLEVER J'AI TROP SOUFFERT=====*/
    ...
    if(panneau.porte_gauche && monster->num_camera == PORTE_BAS){
        attaquer_joueur_echec(map, monster, joueur);
    }
    else if(panneau.porte_droite && monster->num_camera == PORTE_HAUT){
        attaquer_joueur_echec(map, monster, joueur);
    }
