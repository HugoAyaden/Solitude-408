...
int main()
{
    ...
    SDL_Texture *DOOR_R_OFF_L_ON = IMG_LoadTexture(renderer, "./assets/img/INgame/DOO_R_OFF_L_ON.png");
    SDL_Texture *DOOR_L_OFF_L_ON = IMG_LoadTexture(renderer, "./assets/img/INgame/DOOR_L_OFF_L_ON.png");
    ...
    SDL_Texture *DOORS_ON_L_ON = IMG_LoadTexture(renderer, "./assets/img/INgame/DOORS_ON_L_ON.png");
    ...
    SDL_Texture *MONSTER_L_DOOR_O_NA = IMG_LoadTexture(renderer, "./assets/img/INgame/MONSTER_L_DOOR_O_NA.png");
    ...
    SDL_Texture *MONSTER_R_DOOR_O_NA = IMG_LoadTexture(renderer, "./assets/img/INgame/MONSTER_R_DOOR_O_NA.png");
    SDL_Texture *R_DOOR_ON_L_ON = IMG_LoadTexture(renderer, "./assets/img/INgame/R_DOOR_ON_L_ON.png");


    // En cas d'erreur de chargement des assets
    if (!DOORS_OFF_L_OFF || !DOORS_OFF_L_ON || !DOOR_L_OFF_L_ON || !DOOR_R_OFF_L_ON || !DOORS_ON_L_ON || !L_DOOR_ON_L_ON || !MONITOR_ROOM ||
         !MONSTER_L_DOOR_C || !MONSTER_L_DOOR_O_NA || !MONSTER_L_DOOR_O || !MONSTER_R_DOOR_C || !MONSTER_R_DOOR_O_A || 
         !MONSTER_R_DOOR_O_NA || !R_DOOR_ON_L_ON)
    {
    ...
    while (running)
    {
        ...
        while(!timer_monstre()) // 50% de chance de se déplacer de facon optimisee (hint)
            move_monster(carte, &monstre, &joueur);
        else{
            printf("mouv raté\n");
        }

        // GESTION D ELA LUMIERE
        if (lightCount >= 2){
            background = DOORS_OFF_L_ON;
        }
        else if (lumiereGaucheActive && !lumiereDroiteActive ){
            if(monstre.num_camera == carte->cases[X_JOUEUR+1][Y_JOUEUR].num_camera)
                background = MONSTER_L_DOOR_O;
            else
                background = DOOR_L_OFF_L_ON;
        }
        else if (!lumiereGaucheActive && lumiereDroiteActive ){
            if(monstre.num_camera == carte->cases[X_JOUEUR-1][Y_JOUEUR].num_camera)
                background = MONSTER_R_DOOR_O_A;
            else
            background = DOOR_R_OFF_L_ON;
        }
        else
            background = DOORS_OFF_L_OFF;

        //GESTION DES PORTES
        if(lumiereGaucheActive && lightCount == 1 && porteGaucheActive){
            background = L_DOOR_ON_L_ON;
        }
        else if(lumiereDroiteActive && lightCount == 1 && porteDroiteActive){
            background = R_DOOR_ON_L_ON;
        }
        else if(lightCount == 2 && porteGaucheActive && porteDroiteActive){
            background = DOORS_ON_L_ON;
        }
